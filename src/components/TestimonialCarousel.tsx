...
export function TestimonialCarousel({ testimonials }: TestimonialCarouselProps) {
  ...
  return (
    <div className="relative px-2 sm:px-6">
      {/* Left (Voltar) Button - Positioned on same row on the far left */}
      <button
        onClick={() => scroll("left")}
      ...
      {/* Right (Avançar) Button - Positioned on same row on the far right */}
      <button
        onClick={() => scroll("right")}
        disabled={!canScrollRight}
        aria-label="Próximo Depoimento"
        className="absolute -right-2 sm:right-0 top-1/2 -translate-y-1/2 z-20 w-12 h-12 rounded-full bg-orange-500 hover:bg-orange-600 text-white shadow-xl disabled:opacity-0 disabled:pointer-events-none flex items-center justify-center transition-all duration-300 active:scale-95"
      >
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
